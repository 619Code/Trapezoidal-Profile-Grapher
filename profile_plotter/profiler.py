...
def trapizoid_profile_abs(
    ...
    print(f'max_v = {max_v}, max_a = {max_a}, start_v = {start_v}, dx = {dx}')

    # 2 cases: trapezoid/triangle, or line
    if max_v > start_v:
        # We use start_v as a floor, then do the math as if it were zero
        tri_max_height = start_v + (-start_v + math.sqrt(start_v*start_v + 4*max_a*dx)) / 2
        print(f"v_r = {tri_max_height}")

        if tri_max_height > max_v:
            # Trapezoid
            rel_v = max_v - start_v # This doesn't handle the case start_v > max_a
        else:
            # Triangle
            rel_v = tri_max_height - start_v # This doesn't handle the case start_v > max_a
            max_v = tri_max_height
...
def profile():
    # trapizoid_profile_rel(20, 30, 0, 20, 0)
    trapizoid_profile_abs(10, 5, 0, 33, 0)